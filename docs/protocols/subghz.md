...
**Status:** shape `packet` (radio). The `virtual` backend works today; the real `sdr`
backend is not built yet.

Read `../protocol-conventions.md` first. This doc specifies `protocols/subghz.py` and what a
virtual backend must simulate; the same `probe subghz`/`sniff`/`replay` commands drive the future
`sdr` backend unchanged.
...
capture the operator already has and reports a heuristic guess (OOK vs 2-FSK, bitrate,
...
- The sniff client-bound (rule 4) is the key fix this protocol must implement; it is
