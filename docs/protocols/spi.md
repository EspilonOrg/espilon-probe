...
**Status:** shape `transaction`. The `virtual` backend works today; the real `ftdi`
backend is not built yet.

Read `../protocol-conventions.md` first. This doc specifies `protocols/spi.py` and what a
virtual backend must simulate; the same `probe spi *` commands drive the future `ftdi` backend
unchanged.
...
and how an operator reproduces an arbitrary Bus Pirate exchange.
