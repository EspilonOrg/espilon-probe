...
3. `WELCOME` from server: `{version, capabilities}`, where `capabilities` is
   `{protocol, channels, verbs, meta}` (drives `probe info`).
...
- Control (request/response):
  - `SCAN {seconds?, count?}` -> `[{...advertiser/node/device...}]`
    (`seconds`/`count` are OPTIONAL and additive: a listen window and a distinct-device early-stop
    on a packet protocol; an older bridge ignores them and applies its own fixed window)
