...
def _framed(body: bytes) -> io.BytesIO:
    return io.BytesIO(wire._LEN.pack(len(body)) + body)


def test_non_json_body_is_clean_protocol_error():
    # A well-framed body that is not JSON must surface as a clean ProtocolError, not leak the
    # stdlib decoder text ("Expecting value: line 1 column 1 ...").
    try:
        wire.decode(_framed(b"not json at all"))
    except wire.ProtocolError as e:
        assert "malformed frame from target" in str(e)
        assert "Expecting value" not in str(e)
        assert "line 1 column 1" not in str(e)
        return
    raise AssertionError("expected ProtocolError for a non-JSON body")


def test_non_utf8_body_is_clean_protocol_error():
    # A well-framed body with invalid UTF-8 must surface the same clean ProtocolError, not leak
    # "'utf-8' codec can't decode byte 0xff...".
    try:
        wire.decode(_framed(b"\xff\xfe\xfd"))
    except wire.ProtocolError as e:
        assert "malformed frame from target" in str(e)
        assert "codec can't decode" not in str(e)
        assert "utf-8" not in str(e)
        return
    raise AssertionError("expected ProtocolError for a non-UTF-8 body")
