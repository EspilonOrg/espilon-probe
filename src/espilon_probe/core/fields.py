...
def as_float(value, field: str) -> float:
    """Coerce a backend numeric field to float, or raise a clean `ProbeError`.

    Accepts a Python int or float, or a numeric string. Rejects bools, None, and unparseable
    strings with a field-named message rather than leaking `float()`'s own text. An int too
    large to represent as a float (`float(10**400)`) is refused rather than crashing. Non-finite
    inputs (`inf`/`nan`) are returned as-is: they are numerically valid floats, and the single
    place that must not choke on them is where they reach a fixed-width field (see
    `frame._pcap_ts`, which clamps them to a representable pcap timestamp).
    """
    if isinstance(value, bool):
        raise ProbeError(f"backend returned non-numeric {field} {value!r}")
    if isinstance(value, (int, float)):
        try:
            ...
            raise ProbeError(f"backend returned out-of-range {field} {value!r}")
    if isinstance(value, str):
        try:
            return float(value)
...
def hex_bytes(value, field: str) -> bytes:
    ...
    A non-string, or a string that is not valid hex (`"zz"`, an odd length), is refused with a
    field-named message instead of leaking a bare `ValueError`/`TypeError` from `bytes.fromhex`.
    This is the single sound place that hex coercion lives; the protocol modules and the wire
    Frame path route their `raw`/`data`/`value` fields through it so they all fail the same way.
    """
